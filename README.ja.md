...
- デモ動画: [docs/demo-video.mp4](docs/demo-video.mp4)
...
```
GitHub Copilot Chat Setup x.x.x.zip  ← 展開
...
サーバー (http://127.0.0.1:3001) とクライアント (http://localhost:5173) が同時に起動します。
環境によって `127.0.0.1:5173` ではなく `localhost:5173`（または Vite ログの Local URL）でのみ到達できる場合があります。
